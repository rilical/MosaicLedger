...
function maxSeverity(a: OpsSeverity, b: OpsSeverity): OpsSeverity {
...
function severityColor(sev: OpsSeverity): string {
  // Matches the existing Mosaic palette mood (rose/orange/green).
  if (sev === 'high') return '#f43f5e';
  if (sev === 'med') return '#f97316';
  return '#22c55e';
}

function analystLabel(a: OpsAnalyst): string {
  if (a === 'risk') return 'Risk';
  if (a === 'compliance') return 'Compliance';
  return 'Reconciliation';
}

function sortFindingsDeterministic(a: OpsFinding, b: OpsFinding): number {
...
function makeFinding(input: Omit<OpsFinding, 'id'> & { idSeed: string }): OpsFinding {
  return {
...
function vendorConcentration(txns: NormalizedTransaction[]): OpsFinding[] {
  ...
  if (pct < 0.2 || topAmount < 150) return [];

  const severity: OpsSeverity =
    pct >= 0.45 || topAmount >= 1000 ? 'high' : pct >= 0.3 || topAmount >= 500 ? 'med' : 'low';
...
function categorySpike(txns: NormalizedTransaction[], range: DateRange): OpsFinding[] {
  const end = range.end;
  ...
  const baseline = txns.filter(
    (t) => t.amount > 0 && t.date >= baselineStart && t.date <= baselineEnd,
  );

  const sumBy = (items: NormalizedTransaction[]) => {
  ...
  const scored: Array<{ category: string; recentTotal: number; baseTotal: number; ratio: number }> =
    [];
  for (const [cat, recentTotal] of recentBy.entries()) {
    ...
    if (recentTotal < 120) continue;
    if (basePerDay <= 1) continue;
  ...
  for (const x of scored.slice(0, 2)) {
    const severity: OpsSeverity =
      x.ratio >= 3 && x.recentTotal >= 300 ? 'high' : x.ratio >= 2.2 ? 'med' : 'low';
    out.push(
      makeFinding({
        ...
        why: [
          `${x.category} spend spiked ~${x.ratio.toFixed(1)}x in the last 7 days (${formatUsd(
            x.recentTotal,
          )}).`,
...
function duplicateSuspects(txns: NormalizedTransaction[]): OpsFinding[] {
  ...
  for (const [key, arr] of groups.entries()) {
    ...
    const severity: OpsSeverity =
      arr.length >= 3 || total >= 500 ? 'high' : total >= 200 ? 'med' : 'low';
    findings.push(
      makeFinding({
        ...
        entities: { merchant },
        why: [
          `Possible duplicate: ${merchant} ${formatUsd(sample.amount)} on ${sample.date} appears ${
            arr.length
          }x.`,
...
function highFrequencyMerchant(txns: NormalizedTransaction[]): OpsFinding[] {
  const spend = txns.filter((t) => t.amount > 0);
  ...
  for (const x of ranked.slice(0, 3)) {
    const severity: OpsSeverity =
      x.count >= 12 || x.total >= 600 ? 'high' : x.count >= 9 || x.total >= 300 ? 'med' : 'low';
    out.push(
      makeFinding({
...
function roundAmountCluster(txns: NormalizedTransaction[]): OpsFinding[] {
  const spend = txns.filter((t) => t.amount > 0);
  if (spend.length < 12) return [];
  ...
  if (roundCount < 10 || pct < 0.55) return [];

  const severity: OpsSeverity =
    pct >= 0.75 || roundTotal >= 800 ? 'high' : pct >= 0.65 ? 'med' : 'low';
  return [
    makeFinding({
      ...
      entities: {},
      why: [
        `${Math.round(pct * 100)}% of transactions are round-dollar amounts (${roundCount} of ${
          spend.length
        }).`,
...
function buildBriefs(findings: OpsFinding[]): OpsBrief[] {
  const byAnalyst = new Map<OpsAnalyst, OpsFinding[]>();
  for (const f of findings) {
    const arr = byAnalyst.get(f.analyst);
    if (arr) arr.push(f);
    else byAnalyst.set(f.analyst, [f]);
  }

  const analysts: OpsAnalyst[] = ['risk', 'compliance', 'recon'];
  const briefs: OpsBrief[] = [];
  for (const analyst of analysts) {
    const items = (byAnalyst.get(analyst) ?? []).slice().sort(sortFindingsDeterministic);
    if (!items.length) continue;

    let sev: OpsSeverity = 'low';
    for (const f of items) sev = maxSeverity(sev, f.severity);
...
function buildTiles(findings: OpsFinding[]): OpsTileInput[] {
  const tiles: OpsTileInput[] = findings.map((f) => {
