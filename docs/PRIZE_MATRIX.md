...
| **Ripple: XRPL MVP**                    | Round-ups + goal pot + receipts (testnet), with simulation fallback               | Today: deterministic simulation helper exists. Planned: `/app/xrpl` UI + `/api/xrpl/*` routes + receipts list.                                                                                      | Simulation: `NOW`. Testnet send/UI: `PLANNED`.         | `XRPL_TESTNET_SEED`, `XRPL_RPC_URL` (and network).         | Implement round-up compute + send route + UI + receipt log; keep “simulate” always available.         |
| **AppLovin: 15KB mobile browser game**  | Playable, self-contained, <15KB bundle                                            | Not started (current `apps/mobile` is Expo, not a 15KB web game).                                                                                                                                   | `PLANNED`                                              | None.                                                      | Create `/apps/game` tiny web build + size check in CI.                                                |
